...
def get_course(course_id, edu_type=0, term_id_max=100):
    ...
    for i in range(len(files)):
        ...
        thread = threading.Thread(
            target=get_and_make_images, args=(files[i], i,), daemon=True)
    ...
    for i in range(len(labels)):
        for j in range(len(grade_types)-1):
            left[grade_types[j+1]].append(
                left[grade_types[j]][i] +
                grade_list[grade_types[j]][i]
            )
...
if __name__ == '__main__':
    ...
    course = {
        'Nguyễn Thị Ngọc Diệp': {
            'scores': [7.2, 8.0, 8.6, 10.0, 5.1, 9.2, 9.1, 8.2, 6.2, 9.7, 9.3, 9.6, 2.4, 7.6, 7.6, 9.1, 5.8, 7.6, 8.1, 9.1, 7.1, 9.8, 5.3, 9.0, 7.6, 6.6, 6.6, 9.7, 8.1, 9.8, 9.0, 8.1, 8.1, 7.4, 8.1, 6.6, 6.6, 7.6, 8.1, 9.1, 8.1, 7.6, 6.8, 6.8, 6.8, 6.8, 6.2, 6.8, 9.8, 6.2, 6.8, 8.1, 6.8, 6.2, 6.2, 6.2, 9.8, 8.1],
            'extracted': 58,
            'total': 71.0,
            'final_scores': {'A+': 16, 'A ': 1, 'B+': 11, 'B ': 9, 'C+': 11, 'C ': 7, 'D+': 2, 'D ': 0, 'F ': 1}
        },
        'Nguyễn Thanh Thủy': {
            'scores': [8.0, 0.0, 9.0, 0.0, 8.0, 8.0, 8.0, 8.0, 0.0, 7.0, 7.0, 8.0, 8.0, 9.0, 8.0, 8.0, 7.0, 8.0, 8.0, 8.0],
            'extracted': 20,
            'total': 22.0,
            'final_scores': {'A+': 2, 'A ': 0, 'B+': 12, 'B ': 3, 'C+': 0, 'C ': 0, 'D+': 0, 'D ': 0, 'F ': 3}
        },
        'Vũ Thị Hồng Nhạn': {
            'scores': [8.5, 7.0, 6.0, 10.0, 6.0, 6.0, 8.0, 7.0, 7.0],
            'extracted': 9,
            'total': 13.333333333333334,
            'final_scores': {'A+': 1, 'A ': 1, 'B+': 1, 'B ': 3, 'C+': 0, 'C ': 3, 'D+': 0, 'D ': 0, 'F ': 0}
        }
    }
    make_plot(course)
